...
from Backend.commons import ValidSuspect, ValidRooms, ValidWeapons
...
def card_weapon():
    ...
    return Card(ValidWeapons.REVOLVER, CardType.WEAPON)


@pytest.fixture
def card_room():
    """Fixture to create a Room Card instance for testing."""
    return Card(ValidRooms.LIBRARY, CardType.ROOM)
...
def test_deal_card_empty_deck():
    ...
    assert (
        deck.deal() is None
    )  # Should return None when trying to deal from an empty deck
...
def test_add_duplicate_card_to_deck(card_suspect):
    ...
    with pytest.raises(
        ValueError,
        match=f"The card '{card_suspect.get_name()}' is already in the deck.",
    ):
