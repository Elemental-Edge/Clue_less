...
def card_weapon():
    ...
    return Card(ValidWeapons.PIPE, CardType.WEAPON)


@pytest.fixture
def card_room():
    """Fixture to create a Room Card instance for testing."""
    return Card(ValidRooms.BALLROOM, CardType.ROOM)
...
def test_add_duplicate_card_to_hand(card_weapon):
    ...
    with pytest.raises(
        ValueError, match=f"The card '{card_weapon.get_name()}' is already in the hand."
    ):
